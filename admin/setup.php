 ...
 * Copyright (C) 2022 Nicolas SILOBRE <contact@ns-info90.fr>
 *
...
/**
 * \file    nsinfo/admin/setup.php
 * \ingroup nsinfo
 * \brief   Nsinfo setup page.
 */
...
$arrayofparameters = array(
	'NSINFO_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'NSINFO_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/nsinfo/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = GETPOST('object');
	$constforval = 'NSINFO_'.strtoupper($tmpobjectkey)."_ADDON";
...
dol_fiche_head($head, 'settings', '', -1, "nsinfo@nsinfo");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("NsinfoSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'NSINFO_'.strtoupper($myTmpObjectKey).'_ADDON';
